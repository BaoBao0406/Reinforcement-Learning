...
Tuning - version fast - solve as soon as possible
...
def preprocess_frame(frame):
    # Transform the matrix dimision from (84, 84, 1) to (1, 84, 84)
    frame = frame.transpose((2, 0, 1))
    # Convert from numpy to torch format
    frame = torch.from_numpy(frame)
    frame = frame.to(device, dtype=torch.float32)
    # Add one dimision to matrix (1, 1, 84, 84) to fit the NN input format
...
def plot_results():
    ...
    plt.title("Rewards")
    plt.plot(rewards_total, alpha=0.6, color='green', width=5)
...
class NeuralNetwork(nn.Module):
    ...
    def forward(self, x):
        ...
        output_conv = self.conv2(x)
        output_conv = self.activation(output_conv)
        output_conv = self.conv3(x)
        output_conv = self.activation(output_conv)
        
        # Flattern the conv size
...
class QNet_Agent(object):
    ...
    def select_action(self,state,epsilon):
        ...
        if random_for_egreedy > epsilon:      
            ...
            with torch.no_grad():
                ...
                # torch.max(action_from_nn, 1) change from 0 to 1  as after NN, dimision changed
    ...
    def optimize(self):
        ...
        # Convert to List type from tuple
        state = [preprocess_frame(frame) for frame in state]
        # Convert to torch tensor from List type
